...
def predict(image_name):
    ...
    # Decode the predictions to get the class name and prediction probability
    decoded_preds = decode_predictions(preds, top=1)
    class_name = decoded_preds[0][0][1]
    pred_probability = round(float(decoded_preds[0][0][2]), 4)
...
def classify_process():
    ...
    while True:
        ...
        #    so the API can match the results it gets to the original job
        #    sent
        db.set(job_id, json.dumps(prediction_data))
